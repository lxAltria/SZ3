...
namespace SZ {
    ...
    private:
        void embedded_encoding(unsigned char* &c){
            ...
            unsigned char * encoded_num_bitplanes_pos = c;
            unsigned char * encoded_sign_pos = encoded_num_bitplanes_pos + num_blocks;
            unsigned char * encoded_data_pos = encoded_sign_pos + (((n - 1) / block_size + 1) * (block_size / 8));
            ...
            if(n >= block_size){
                for(; i<n - block_size; i+=block_size){
                    ...
                    for(int j=0; j<block_size; j++){
                        if(fabs(data_pos[j]) > max_val){
                            max_val = fabs(data_pos[j]);
                        }
                    }
                    int data_exp = 0;
                    ...
                    int num_bitplanes = data_exp - eb_exp;
                    if(num_bitplanes <= 0){
                        *(encoded_num_bitplanes_pos ++) = 0;
                        continue;
                    }
                    else{
                        *(encoded_num_bitplanes_pos ++) = (unsigned char) num_bitplanes;
                    }
        ...
        void embedded_decoding(const unsigned char * &c, size_t unpred_size){
            ...
            const int num_blocks = (n - 1) / block_size + 1;
            uint64_t int_data_buffer[block_size];
            unsigned char const * encoded_num_bitplanes_pos = c;
            unsigned char const * encoded_sign_pos = encoded_num_bitplanes_pos + num_blocks;
            unsigned char const * encoded_data_pos = encoded_sign_pos + (((n - 1) / block_size + 1) * (block_size / 8));
            ...
            if(n >= block_size){
                for(; i<n - block_size; i+=block_size){
                    int num_bitplanes = *(encoded_num_bitplanes_pos ++);
                    if(num_bitplanes){
